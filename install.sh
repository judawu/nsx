...
xray_config() {
    ...
    jq -c '.inbounds[] | select(.settings.clients)' "$TEMP_FILE" | while IFS= read -r inbound; do
        ...
        if [[ "$security" == "reality" ]]; then
            ...
            if [[  "$tls_ech" == "y" ]]; then
                ...
                url="$url&security=tls&fp=chrome&sni=$sni&alpn=$alpn&ech=$echConfigList&pcn=$pcn"           
            else
    ...
    if [[ -z "$split_ss" ]]; then
       ...
       jq -c '.outbounds[] | select(.protocol=="shadowsocks")' "$TEMP_FILE" | while IFS= read -r outbound; do
          ...
          read -p "输入 shadowsocks address 默认($ss_address): " ss_new_address < /dev/tty
          if [[ -z "$ss_new_address" ]]; then
            ss_new_address="$ss_address"         
          fi
           read -p "输入 shadowsocks method 默认($ss_method) 0=2022-blake3-aes-128-gcm, 1=2022-blake3-aes-256-gcm, 2=2022-blake3-chacha20-poly1305: " ss_method_option < /dev/tty
           case "$ss_method_option" in
                    0) 
...
configufw(){
    ...
    sudo ufw allow 5353
    sudo ufw allow 6753
    sudo ufw allow 4001,4002,4003,4004,4005,4005,4007,4008,4009,4010/tcp
    sudo ufw allow 4001,4002,4003,4006,4011,4012,4013,4014,4015,4016/udp
    sudo ufw allow 4011,4012,4013,4014,4015,4016,4017,4018,4019,4020/tcp
    sudo ufw allow 8071,8072,8073,8074,8075,8076,8077,8078,8079/tcp
    sudo ufw allow 8071,8072,8073,8074,8075,8076,8077,8078,8079/udp
    sudo ufw allow 8080,8081,8082,8083,8084,8085,8086,8087,8088,8089,8090/tcp
    sudo ufw allow 8080,8081,8082,8083,8084,8085,8086,8087,8088,8089,8090/udp
    ...
    sudo ufw allow 10830,10831,10832,10833,10834,10835,10836,10837,10838,10839,10840/tcp
    sudo ufw allow 10830,10831,10832,10833,10834,10835,10836,10837,10838,10839,10840/udp
    sudo ufw allow 10841,10842,10843,10844,10845,10846,10847,10848,10849,10850,10851/tcp
    sudo ufw allow 11111,22222,33333,44444,55555,1111,2222,3333,4444,5555,6666/tcp
    sudo ufw allow 7777,8888,9999,10000,10001,20000,30000,40000,50000/tcp
    sudo ufw allow 3344,3443,4443,4434,8443,4433,4455,5544,5566,6655,4466,6644/tcp
    sudo ufw allow 3344,3443,4443,4434,8443,4433,4455,5544,4466,6644,5566,6655/udp
